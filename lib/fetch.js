...
const fetch = (url) => {
  return new Promise((resolve, reject) => {
    ...
    feedparser.on('error', (e) => {
      ...
      if (!feedparser.meta.updatedAt || isNaN(feedparser.meta.updatedAt.getTime())) {
        log('warning: %s seems not standard pubDate. trying to parse using moment...', url);
        ...
        if (pubDate.isValid()) {
          log('woo, moment parsed date! (original: %s, parsed: %s)', _pubDate['#'], pubDate.format('YYYY-MM-DD HH:mm:ss'))
          feedparser.meta.updatedAt = pubDate.toDate();
        }
      }

      if (!feedparser.meta.updatedAt || isNaN(feedparser.meta.updatedAt.getTime())) {
        log('warning: %s seems not standard pubDate. trying to use record pubDate...', url);
        ...
        if (pubDate && !isNaN(pubDate.getTime())) {
          log('woo, got pubDate from first record! (%s)', pubDate);
      ...
      if (!feedparser.meta.updatedAt || isNaN(feedparser.meta.updatedAt.getTime())) {
        ...
        log('error: %s seems not standard pubDate. failed to reading pubDate!', url);
